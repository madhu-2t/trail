...
app.put('/joke/:id',(req,res)=>{
  const stg=parseInt(req.params.id);

  jokes[stg-1]["jokeText"]=req.body.text;
  jokes[stg-1]["jokeType"]=req.body.type;
  // console.log(jokes[stg]);
  res.json(jokes[stg-1]);

});
app.patch('/joke/:id',(req,res)=>{
  const stg=parseInt(req.params.id);
  const parameters=req.body;
  const change=Object.keys(parameters)[0];
  console.log(parameters);
  console.log(change);
  console.log(req.body.change);
  if(change==='type'){
    jokes[stg-1]['jokeType']=req.body[change];
  }
  else{
    jokes[stg-1]['jokeText']=req.body.text;
  }
